...
Classification of the Z2L_Z2 T-folds on the SO(12) lattice from scratch:
  ...
  2. reduce the survivors modulo E1-E3 with the exhaustive G_L x G_R search of
    ...
    python classify_Z2L_Z2.py [--out-dir DIR] [--no-all-mi]
"""
import argparse
import os
import sys 
...
def main():
    ...
    CT.write_classes(res, os.path.join(args.out_dir, "Z2L_Z2_inequivalent.csv"),
                     SPEC.param_cols)
    if not args.no_all_mi:
        CT.write_all_MI(res, os.path.join(args.out_dir, "Z2L_Z2_all_MI.csv"),
                        SPEC.param_cols)
